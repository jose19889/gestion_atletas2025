...
<head>
 ...
 <title>GESTION DE ATLETAS</title>
...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <?php
        ...
        // Validación por si el usuario no está en sesión aún
        if ($user && is_array($user)) {
            $username = $user['username'];
            $roleName = $user['role_name'];
            $userId = $user['id'];

            // Cargar imagen del perfil desde base de datos
            $userModel = new \App\Models\UserModel();
            $userRecord = $userModel->find($userId);

            if ($userRecord && isset($userRecord['profile_image']) && !empty($userRecord['profile_image'])) {
                $imageUrl = base_url('uploads/profiles/' . $userRecord['profile_image']);
            } else {
                $imageUrl = base_url('uploads/img/avatar5.png');
            }
        } else {
            // Si no hay usuario en sesión, usar datos por defecto
            $username = 'Invitado';
    ...
    <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right" style="left: inherit; right: 0px;">
        ...
        <li class="user-body">
            <div class="row">
                <div class="col-12 text-center">
                    <a class="text-danger" href="<?= site_url('users-changePass') ?>">Cambiar Contraseña</a>
                </div>
            </div>
        </li>
        ...
        <li class="user-footer">
            ...
            <a href="<?= site_url('logout') ?>" class="btn btn-default btn-flat float-right">Salir</a>
        </li>
    </ul>
</li>
