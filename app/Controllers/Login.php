...
class Login extends BaseController
{
 ...
 public function auth(){ $session = session();
$model = new UserModel();
...
if ($data) {
    ...
    if ($verify_pass) {
        ...
        $ses_data = [
            ...
            'club_id' => $data['club_id'], // asegúrate de tener esto
            'profile_image'=> isset($data['profile_image']) ? $data['profile_image'] : 'default.png', // Default image
             'role_name'=>$data['role_name'],
            'logged_in'=> TRUE
        ];

       // $session->set($ses_data);
       $session->set(['user' => $ses_data]);
...
{
    $session = session();
    $session->destroy(); // Destruye todos los datos de sesión
    return redirect()->to('/login');
}
